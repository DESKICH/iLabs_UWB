...
void spiBegin(uint8_t irq, uint8_t rst, uint8_t wake)
{
  ...
  */
  ...
  // Wakeup pin not used in library but needs to be set to high for operation.
  _wake = wake;
  pinMode(wake, OUTPUT);
...
int readfromspi(uint16_t headerLength, uint8_t *headerBuffer, uint16_t readLength, uint8_t *readBuffer)
{
  ...
  for(int i = 0; i < headerLength; i++) {
    SPI1.transfer(headerBuffer[i]); // send header
  }
  for(int i = 0; i < readLength; i++) {
    readBuffer[i] = SPI1.transfer(JUNK); // read values
  }
  delayMicroseconds(5);
  digitalWrite(_ss, HIGH);
  SPI1.endTransaction();
...
int writetospi(uint16_t headerLength, uint8_t *headerBuffer, uint16_t bodyLength, uint8_t *bodyBuffer)
{
  ...
  for(int i = 0; i < bodyLength; i++) {
    SPI1.transfer(bodyBuffer[i]); // write values
  }
  delayMicroseconds(5);
  digitalWrite(_ss, HIGH);
  SPI1.endTransaction();
...
void port_set_dw_ic_spi_fastrate(uint8_t irq, uint8_t rst, uint8_t ss, uint8_t wake) {
    spiBegin(irq, rst, wake);
    spiSelect(ss);
}
